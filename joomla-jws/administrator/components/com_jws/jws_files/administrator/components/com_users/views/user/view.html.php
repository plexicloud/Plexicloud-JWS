...
class UsersViewUser extends JView
{
	function display($tpl = null)
	{
		...
		*/
		...
		if($edit) {
			$sql = "select apikey, secretkey, blocked from #__users_apikey where userid=".$cid[0];
			$db = & JFactory::getDBO();
			$db->setQuery($sql);
			$obj = $db->loadObject();
		...
		$lists['api'] = JHTML::_('select.booleanlist',  'api', 'class="inputbox" size="1"', $user->get('_blocked') );
